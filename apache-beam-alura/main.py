...
dengue = (
    pipeline
    | 'Leitura do dataset de sample_dengue' >> ReadFromText('data/sample_casos_dengue.txt', skip_header_lines=1)
    ...
    # Pega todas as chaves que são iguais e soma os valores
    | 'Soma dos casos pela chave' >> beam.CombinePerKey(sum)
    # | 'Exibir na tela' >> beam.Map(print)
    #   # | 'Salvar em arquivo local' >> WriteToText('data/casos_por_uf_data.txt')
)
...
chuvas = (
    pipeline
    | 'Leitura dataset sample_chuvas' >> ReadFromText('data/sample_chuvas.csv', skip_header_lines=1)
    ...
    # | 'Salvar em arquivo local' >> WriteToText('data/teste.txt')
)
...
resultado = (
    ...
    # Utilizar um novo método que realiza o empilhamento e o agrupamento de uma vez.
    ...
    # | 'Salvando pasta local' >> WriteToText('data/resultado_sample_chuva_e_dengue', file_name_suffix='.csv')
)
